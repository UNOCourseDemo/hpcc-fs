...
The five heuristics span the sender's design space: a $k$-aware additive boost scaled by the
congested-hop count ($1.63\times$, the best case, at $\gamma{=}50$); a debiased max/mean blend
of per-hop $u_i$ ($1.95\times$ --- per-hop $u_i$ are nearly equal here, no max-bias to debias);
a responsibility-weighted multiplicative decrease ($1.89\times$ --- the MD branch barely fires
at $\eta$-hold); a $k$-th-root MD ($1.92\times$); and a share-aware additive increase on every
flow ($1.71\times$). Stock is $1.96\times$; the target is $1.0$.
...
RCP~\cite{dukkipati2006fct,dukkipati2008rcp} and XCP~\cite{katabi2002xcp} place the computation
...
its queue near zero and forfeits round-robin turns). This is the paper's theme inverted: RCP's
fixed point presumes every flow on the port obeys $R$; non-compliant flows are invisible to the
inference exactly as flow count is invisible to a sender.
...
FCT (oracle $=1.0$); \emph{oracle-normalized penalty} = per-flow FCT / fluid max-min oracle FCT;
\emph{generalized unfairness} (heterogeneous cases) = worst-long / best-flow penalty;
\emph{raw long/short ratio} = mean long / mean short FCT (ECMP study); \emph{slowdown} = FCT /
same-size standalone FCT; \emph{coflow JCT} = max FCT over a coflow.
...
only HPCC and the RCP mode hold zero PFC. We do not extend the claim to
untested controllers (PowerTCP, Bolt, On-Ramp, Swift), whose feedback models differ materially;
a same-benchmark comparison is future work.
...
cross-pod, multi-bottleneck ones stock HPCC starves. We run a 16-flow ring allreduce over all
hosts of the $k{=}4$ fat-tree (4 inter-pod, 4 intra-pod, 8 same-edge segments; 50 MB each)
across the same five ECMP hash seeds. \hpccfs cuts JCT by $19.5$--$22.7\%$ (mean $21.4\%$) at \emph{every}
...
schedule; \emph{generalized unfairness} = worst-long / best-flow penalty (each vs.\ its own
oracle FCT), which reduces to the headline ratio in the uniform case. Stock HPCC's unfairness
\emph{grows} with contention richness --- worst with two overlapping long flows ($2.87\times$)
and churn ($2.23\times$) --- while \hpccfs holds $1.001$--$1.007\times$ throughout, and
$1.003$--$1.008\times$ under $4$--$8\times$ RTT spreads applied to the mode itself; PFC $= 0$
everywhere. (HPCC's INT format admits only the discrete rates $\{25,50,100,200,400\}$ Gbps;
capacity heterogeneity is exercised within that set.)

\begin{table}[t]
\caption{Generalized unfairness (worst-long / best-flow, oracle-normalized) beyond the uniform
parking lot. \hpccfs holds ${\approx}1.0\times$ in every scenario; PFC $= 0$ throughout.}
\label{tab:hetero}
\centering\small
\begin{tabular}{lcc}
\toprule
scenario & stock HPCC & \hpccfs \\
\midrule
hetero capacities [25,50,25,100] G & $1.82\times$ & $\mathbf{1.003\times}$ \\
unequal competitors [3,1,2,1] & $1.41\times$ & $\mathbf{1.001\times}$ \\
two overlapping long flows & $2.87\times$ & $\mathbf{1.007\times}$ \\
cross-flow churn (arr./dep.) & $2.23\times$ & $\mathbf{1.002\times}$ \\
\bottomrule
\end{tabular}
\end{table}
...
saturating workload is future work. (iii)~\emph{ECMP placement}: a raw ${\sim}1.3\times$ long/short spread from hash placement
...
(\S\ref{sec:integration}); \hpccfs is a fabric-wide mode.
...
dropping; programmable schedulers~\cite{sivaraman2016pifo} realize such policies at line rate.
\hpccfs instead \emph{advertises} one fair rate per port for rate-controlled senders to adopt
--- no per-flow tracking, no extra queues --- implementable on the match-action
pipelines~\cite{bosshart2014p4} HPCC's INT already assumes.
...
\paragraph{Fairness foundations.} Max-min, progressive filling, and the parking lot are
classical~\cite{bertsekas1992data}; our contribution is the measurement, not a new definition.
...
schemes' penalty ${<}1$; \hpccfs adds no overhead, 0 PFC) --- a saturating workload is future
work. (Capacity, competitor, and RTT heterogeneity are now exercised directly in
\S\ref{sec:hetero}.) The path-min aggregation is path-invariant under ECMP; residual FCT spread
is load placement, a routing issue. On
...
belt-and-braces run on the unmodified legacy build remains future work. We did, however,
