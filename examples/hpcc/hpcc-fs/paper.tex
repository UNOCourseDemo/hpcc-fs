...
not belong: it cannot share an unsliced link with stock HPCC, and it sustains only
$25$--$50\%$ utilization at fan-in past ${\sim}64$. Stock HPCC's code remains byte-for-byte
unchanged.
\end{abstract}
...
\emph{Nor a tuning artifact:} sweeping stock HPCC's own parameters at $N{=}4$ ---
$\eta \in \{0.90, 0.95, 0.98\}$, $R_{AI}$ up to $25\times$ its reference (1 Gb/s), and
min-rate --- leaves the ratio at $1.74$--$1.97\times$, PFC $0$ (\texttt{run\_round5.py});
...
convergence takes several control intervals ($5\%$ by ${\sim}0.6$\,ms, $1\%$ by
$2.3$\,ms; \S\ref{sec:eval}) --- fast and path-length-independent.
...
conservative for our claim. Both regression
...
$5$--$13\%$ of its own oracle FCT --- and since the wire-overhead calibration alone is
$5.8\%$, the measured convergence residual is ${\approx}0$ outside churn (whose $1.13$ worst
case carries real dynamics). Mean bottleneck utilization is \emph{higher} than stock in all
four scenarios ($0.65$--$0.78$ vs
...
\emph{Incast fan-in, and the rate floor} (\texttt{run\_round4.py}, \texttt{run\_round5.py}):
$S$ senders to one 25 Gbps receiver link. Fan-in
...
C/R_{\mathrm{floor}} = 250$ at $100$ Mbps; at $S{=}256$ senders pin at the floor (utilization
$0.99$ only because floor ${\approx}$ fair share there); beyond, the port is structurally
overloaded. High fan-in is a
standing limitation of the evaluated gains (\S\ref{sec:scope}(v)); stabilizing the loop at
large $N$ (longer load averaging, $N$-aware gains) is future work. The floor never binds when
fair shares exceed it; every fairness result reproduces exactly.
...
(\S\ref{sec:integration}); \hpccfs is a fabric-wide mode. (v)~\emph{High fan-in}: at $S \ge 64$ the rate mode sustains only $25$--$50\%$
utilization regardless of flow size, and $S \le C/R_{\mathrm{floor}}$ is a hard bound
(\S\ref{sec:fanin}); combined with (iv), fan-in-heavy services belong on the stock side of
...
behavior is unknown; we leave a head-to-head to future work.
...
fair-rate update; whether the three logical state values and the two multiplies fit within one
target's stateful pipeline stage is \emph{unverified}, as are dequeue-time queue-depth access
and quantization behavior under fixed-point arithmetic (plausible widths: 32-bit $R$ in Mbps,
byte counters, wrap-safe ns timestamps; 24-bit queue depth; a Q16 multiplier with saturating
clamp). A fixed-point simulation and a Tofino prototype remain future work.
