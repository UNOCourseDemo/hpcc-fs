...
\title[Multi-Bottleneck Fairness on RDMA Fabrics]{Multi-Bottleneck Fairness on RDMA Fabrics: An Empirical Study}

\subtitle{HPCC's Failure, the Limits of Sender-Side Control, and RDMA-RCP: an RCP Service Mode over In-Network Telemetry}
...
scalar of state per port --- restores the max-min allocation (penalty ${\approx}1.005\times$)
...
so per-class deployment needs sliced telemetry or a separate fabric. Stock HPCC remains
byte-for-byte unchanged alongside.
\end{abstract}
...
We proceed with a deterministic, oracle-grounded methodology. Our congestion-control stack is a
...
64-bit INT field with one scalar of state per egress port, selected per traffic class alongside
...
reservation, because both controllers still read port-global telemetry
(\S\ref{sec:integration}); per-class deployment needs sliced signals or a separate fabric. The mechanism is RCP --- we claim
...
shared link the local single-bottleneck flow wins and the long flow is squeezed.
...
\subsection{The strongest sender-side design: a virtual-RCP mirror}
\label{sec:vrcp}

The five variants above are heuristics, and a fair objection is that they do not exhaust the
design space. The strongest sender-side design is implied by \S\ref{sec:design} itself: RCP's
inputs --- capacity $C$, aggregate load $y$, queue $q$ --- are all sender-visible in HPCC's INT
record. So let the sender run the switch's recursion \emph{privately}: per hop, maintain a
virtual fair rate $\hat R_i$ updated by the identical RCP rule from the flow's own INT samples,
and send at $\min_i \hat R_i$ (\texttt{mb\_mode 6}; same gains, same $R_0 = C/2$, the sender's
own base RTT as control interval --- the settings most favorable to the endpoint). If the
obstacle were \emph{information}, this variant would match the switch.

It does not (Table~\ref{tab:vrcp}). With synchronized starts it reaches only $1.56\times$; under
staggered arrival it lands at $1.65\times$ or $0.81\times$ --- the \emph{direction} of the
unfairness set by arrival order --- while the identical law at the switch holds
$1.003$--$1.008\times$ in all three scenarios. The mechanism is visible in the update itself:
the recursion is multiplicative, so senders driven by a common observed $(y, q)$ sequence
multiply their private estimates by (nearly) the same factor, and the \emph{ratio} between their
estimates never contracts. A late-arriving flow initializes fresh while incumbents hold evolved
state, and no later observation reconciles them; RTT heterogeneity alone --- the synchronized
case --- already breaks the common trajectory.
...
\texttt{cc\_mode 11} the INT wire format is a global choice, so a fabric runs one mode at a time.
To test the per-class deployment the framing suggests, we added \texttt{cc\_mode 12}: a wire layout
identical to stock HPCC's 42-byte INT record (so stock packets stay byte-for-byte unchanged), with
the switch and sender dispatching \emph{per packet} on the traffic class. The FS class reuses the
record's first eight bytes for the fair rate and never pushes hop records; it therefore pays stock's
header overhead, which is conservative for our claim. Two regression gates hold: with every flow in
the stock class, \texttt{cc\_mode 12} reproduces \texttt{cc\_mode 3}'s FCTs \emph{exactly}; with
every flow in the FS class it reproduces \texttt{cc\_mode 11}'s $1.005\times$ penalty at zero PFC.
...
\subsection{The failure is endemic: deployable baselines}
\label{sec:endemic}
...
configurations). Table~\ref{tab:endemic}: \emph{every} scheme is unfair to the multi-bottleneck flow and worsens
with bottleneck count --- DCQCN worst at $N{=}4$ ($2.32\times$). HPCC is the norm, not an
outlier; only the RCP mode reaches max-min, and only HPCC and the RCP mode hold zero PFC.
...
load-bearing; we do not claim the same for arbitrary topologies or joint sweeps.
...
variants $1.002$--$1.009\times$, all PFC $= 0$ (a second operating point; the headline runs use the 1\,Gbps startup).
...
deployable member of the family fails it.
...
stack. ABC~\cite{goyal2020abc} argues, for wireless
...
schedulers~\cite{sivaraman2016pifo,yu2021aifo} realize such policies at line rate. \hpccfs instead
...
\paragraph{Relation to HPCC, and a hybrid path.} \hpccfs demonstrates \emph{where} the
fair-share signal must live; it inherits RCP's character --- fair, low-queue --- without HPCC's
aggressive single-flow precision (\S\ref{sec:cost}). A natural future hybrid runs HPCC unchanged
and applies the switch's fair rate only as a per-flow \emph{ceiling},
$\min(\mathit{rate}_{\mathrm{HPCC}}, \mathit{fairRate})$; the open obstacle is HPCC's NIC-sized
window, which can hold a victim below fair even after the ceiling frees bandwidth
(\S\ref{sec:ablation} suggests a fair-rate-scaled window as the fix).

\paragraph{INT hop limit.} Stock HPCC's per-hop INT cost grows with path length and its
reference format caps at $\texttt{maxHop} = 5$, corrupting the control input beyond four
bottlenecks; \hpccfs's one-field format is hop-count-independent and stays at
${\approx}1.0\times$ at $N = 5, 6$.
...
under ECMP (\S\ref{sec:eval}); residual FCT spread is load placement, a routing issue. In
hardware, the per-port update is three multiplies and an add per control interval, and the
per-packet min-aggregation is a 64-bit compare-and-write --- well within programmable-switch
budgets.
...
with penalty $1.005\times$ and zero PFC at $N = 4$ across all workloads we tested, converging
sub-millisecond and independent of path length, while leaving the original HPCC code path unchanged.
