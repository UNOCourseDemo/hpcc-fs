...
magnitude too slow (we claim empirical evidence, not formal impossibility). We argue the fix
belongs at the switch: \hpccfs
...
On contention benchmarks it drives the multi-bottleneck penalty to ${\approx}1.0\times$ across
bottleneck counts, asymmetric flow sizes, staggered starts, and ECMP hash seeds, with zero PFC;
on allreduce-style coflows --- where the job is gated by its slowest, multi-bottleneck flows ---
it cuts job completion time by ${\sim}20\%$ at every ECMP seed. We delimit the claim explicitly
where max-min is not the operator's objective (mixed short-flow
workloads) and where fabric-scale contention does not form. The mechanism is RCP; our primary
contribution is the \emph{diagnosis} and the demonstration that the fix belongs in the network,
not the endpoint. The stock HPCC code path is left byte-for-byte unchanged.
\end{abstract}
...
where the subsidy inverts into a job-level tax (\S\ref{sec:jct}). (ii)~\emph{Fabric scale}: at $k = 6, 8$
our scaling workload does not form persistent inter-pod contention (both schemes' penalty
${<}1$), so the fix is exercised, but untested, where large-fabric contention would arise; a
