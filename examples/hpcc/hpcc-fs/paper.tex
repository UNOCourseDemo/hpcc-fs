...
starved by ${\sim}2\times$ relative to single-bottleneck competitors, and HPCC's control input is
corrupted outright once the per-hop INT record overflows its hop budget --- a cap inherited from
HPCC's original wire format.
...
multi-bottleneck max-min fairness with zero PFC: one 64-bit header field and one fair-rate scalar (three words of state)
...
--- it cuts job completion time by ${\sim}21\%$ (mean; $19.5$--$22.7\%$) at every ECMP seed. We state explicitly when the
...
We address that question with a deterministic, oracle-grounded methodology. Our
congestion-control stack is a refactored re-port of the original HPCC ns-3 simulator onto a tagged
ns-3.45 release; before drawing any conclusion from it we validated the re-port by reproducing the
original paper's baseline comparisons across five schemes and two production workloads
(\S\ref{sec:eval}), and the full reproduction record and raw results are public. Our
contributions:
...
coflows where the slowest flow gates the job, a ${\sim}21\%$ mean JCT reduction at every ECMP hash
seed (\S\ref{sec:jct}).
\end{enumerate}
...
cross flows hold ${\sim}23.2$ Gbps each for the entire 19 ms contention window; only after the
...
at startup, they keep it. Additive nudges (modes 1, 5) take orders of magnitude more RTTs than the flow's lifetime (our 200\,ms runs are ${\sim}10^4$ RTTs, and modes 1 and 5 never converge) to
...
The empirical and structural arguments of \S\ref{sec:senderonly} motivate a design that (a)
...
several control intervals (Section~\ref{sec:eval}: all five flows are within $5\%$ of the fair
share by ${\sim}0.6$\,ms, i.e.\ ${\sim}30$ RTTs at our 21\,$\mu$s base RTT, and within $1\%$ by
${\sim}2.3$\,ms). HPCC-FS's advantage is therefore \emph{fast, path-length-independent}
...
On the $N{=}4$ parking lot we then placed the multi-bottleneck long flow in the FS class and the
four cross flows in the stock class, sharing the same links. \emph{Neither class gets its fair
share.} Rate-only FS (our default) overshoots: the long flow takes $21.6$ Gbps against a $12.5$ Gbps
fair share and pushes the stock cross flows down to $11.1$ Gbps (penalty $0.51\times$, i.e.\ the
starvation reverses). Re-enabling the FS window instead starves the FS flow ($6.2$ Gbps,
$2.53\times$ --- worse than stock HPCC's own $1.96\times$). Giving the FS class its own priority
queue does not help ($0.51\times$): HPCC drives its queue to near-zero by design, so it forfeits its
round-robin turns to a neighbour that maintains a standing queue. The cause is the paper's own theme
turned on its head: an RCP port infers the fair share from aggregate load, so its fixed point
$R = C/N$ presumes that \emph{every} flow on the port obeys $R$. Flows running a different control
law are invisible to that inference in exactly the way flow count is invisible to a sender.

The deployable reading is therefore narrower than ``two modes on one link'': \hpccfs needs a reserved
share of capacity (a scheduler weight or a separate fabric), not free competition with stock HPCC.
Bandwidth-partitioned coexistence and an RCP variant that estimates the compliant-flow share are
future work. All variants ship with the artifact (\texttt{run\_mixed\_mode.py}); PFC is zero in every
one.
...
the WebSearch mix of \S\ref{sec:scope}), and max-min removes the lottery. The 8-flow ECMP
workload of \S\ref{sec:eval} shows the same job-level effect when read as a coflow: its makespan
improves $9.8$--$26.5\%$ across seeds (\texttt{run\_ecmp\_seeds.py} runs; ring workloads in
\texttt{run\_allreduce\_jct.py}).
...
oracle-normalized penalty is already $1.001\times$, \hpccfs moves to $0.581\times$, and on mean
slowdown \hpccfs trades ${\sim}15\%$ off multi-bottleneck flows for ${\sim}45\%$ on short paths
(a mean dominated by sub-10\,KB flows, \S\ref{sec:discussion}). Workloads whose objective is
short-flow FCT (the Homa/pFabric regime~\cite{montazeri2018homa,alizadeh2013pfabric}) may prefer
that subsidy; max-min is the right target when multi-bottleneck flows matter --- in particular when
they gate a collective, where the subsidy inverts into a job-level tax (\S\ref{sec:jct}). This is a
per-class \emph{mode-selection} question, not a global replacement. (ii)~\emph{Fabric scale}: at $k = 6, 8$
...
(\S\ref{sec:eval}), not the placement component, which is a routing/load-balancing problem. (iv)~\emph{Coexistence}: on an \emph{unreserved}
shared queue the two modes do not each receive their fair share (\S\ref{sec:integration}); \hpccfs
needs a reserved bandwidth share.
...
keeping HPCC's single-rate sender and per-hop transport. \hpccfs's per-port fair-rate update and path-min aggregation \emph{are} RCP's; the differences
from canonical RCP are at the endpoint and in the control interval, not in the fair-rate law. Our
sender is rate-only --- it does not set the RCP window $\mathit{cwnd} = R\cdot\mathrm{RTT}$
(\S\ref{sec:design}; re-enabling any window costs the fairness result, Table~\ref{tab:window})
--- the control interval is a fabric-wide constant rather than a measured RTT average, the update is
event-driven on dequeue, and the port applies a startup value $R_0 = C/2$ and a hard rate floor. A
standalone-RCP head-to-head would therefore differ chiefly in the window and the interval; we
\emph{expect}, but do not claim, the same fairness numbers. What \hpccfs adds over deploying RCP as
a separate protocol is packaging, not control: one field on the INT transport a datacenter already
runs, a byte-identical stock HPCC path, and per-\texttt{cc\_mode} mode selection --- no second
protocol stack, and no growth in the INT header. ABC~\cite{goyal2020abc} argues, for wireless
...
diagnosis that five representative sender-side corrections fail to realize this well-understood
objective from sender-visible INT alone, and the demonstration that a single switch-computed fair
rate achieves it on our benchmarks.
...
testbed measurements; per-class coexistence needs a reserved bandwidth share, which we do not design
here (\S\ref{sec:integration}); convergence is sub-millisecond but not one RTT (\S\ref{sec:design}); and the ECMP
...
(\S\ref{sec:integration}): the mode belongs on a reserved share, not in free competition.
