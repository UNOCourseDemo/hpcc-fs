...
We measure multi-bottleneck fairness on lossless RDMA fabrics against a fluid max-min
bandwidth-allocation bound: every scheme we evaluate (DCQCN, TIMELY, DCTCP, HPCC) is unfair to
the multi-bottleneck flow ($1.6$--$2.3\times$), worsening with bottleneck count; HPCC's
instance is a stable near-winner-take-all collapse to ${\sim}0.4$ Gbps. Six sender-only
corrections fail --- including a private mirror of the RCP recursion driven by the same INT
signals: independently maintained explicit-rate state preserves arrival-history asymmetry,
while one shared per-link register erases it --- and per-flow fair queueing equalizes only
what the endpoint offers. Carrying that register's value in one 64-bit INT field
(\hpccfs, three words of state per port) restores max-min --- $1.005\times$, every flow
within $5$--$13\%$ of its oracle FCT in heterogeneous scenarios ($21$--$39\%$ under
compound stress), zero PFC --- and cuts a ring-collective coflow's completion time by
$18$--$23\%$ ($18\%$ under byte-identical headers). We equally measure where the mode does
not belong: it cannot share an unsliced link with stock HPCC, and fan-in past ${\sim}64$
needs a lengthened control interval ($25$--$50\%$ utilization at the default; $84$--$94\%$
at $2$--$4\times$). Stock HPCC's code remains byte-for-byte unchanged.
\end{abstract}
...
\emph{Nor a tuning artifact:} sweeping $\eta \in \{0.90, 0.95, 0.98\}$, $R_{AI}$ to
...
blast before any ACK returns; adopted rates may fall to $R_{\mathrm{floor}}$ thereafter.
...
same-size standalone FCT; \emph{coflow JCT} = max FCT over a coflow. ``Zero PFC'' statements refer to the evaluated
buffer/PFC configuration (\S\ref{sec:queue}).
...
intra-pod, 8 same-edge) over all hosts of the $k{=}4$ fat-tree, across five ECMP seeds. A full allreduce chains $2(N{-}1)$ such phases. Chaining four
barrier-synchronized $3.125$ MB phases (each starting at the prior phase's measured
completion, controller state carried) gives $21.6\%$ cumulative vs $22.1\%$ single-phase ---
savings accumulate, \hpccfs's phase JCTs stable at $1.19$ ms (\texttt{run\_round7.py});
compute overlap unmodeled. \hpccfs cuts JCT by $19.5$--$22.7\%$ (mean $21.4\%$) at \emph{every}
...
$5$--$13\%$ of its own oracle FCT --- the wire calibration alone is $5.8\%$, so the
measured convergence residual is ${\approx}0$ outside churn ($1.13$ worst case). Mean
...
$1.000\times$ while absolute $P_i$ rise to $1.21$--$1.39$ --- uniform slowdown the absolute
metric exposes and the ratio would hide (\texttt{run\_round3.py}). (INT admits only rates $\{25,50,100,200,400\}$ Gbps; heterogeneity stays within that
set.)
...
of bins below $C/2$ (\texttt{run\_round6.py}). The damping
knob exists: interval $\times 2$ (\texttt{fs\_d\_scale}) restores $0.84$--$0.88$ at
$S{=}64$ (both sizes, PFC-free); $\times 4$ reaches $0.87$--$0.94$ (44 pauses at $S{=}64$,
none at $S{=}128$)
--- with the $N{=}4$ headline unchanged ($1.003\times$, PFC 0; cf.\
\S\ref{sec:sensitivity}). A residual gap to stock
HPCC's $0.92$ remains at the default interval; adaptive interval selection is future work.
The floor never binds when fair shares exceed it; every fairness result reproduces
exactly.
...
($R_0{=}C/2$ ramp): the drain tail's last dequeues restore $R$ to ${\approx}C$. The frozen value is benign after
...
15 transient pauses). The mode fits services gated by the collective.
...
common, so the delta is attributable to the controller in aggregate (startup, queueing,
utilization, fairness) --- a scale-and-performance sanity check, \emph{not} a max-min
validation; a path-resolved oracle needs path instrumentation ($k{=}8$: future work). (iii)~\emph{ECMP placement}: a raw ${\sim}1.3\times$ long/short spread from hash placement
...
same 32-bit register remove it. Register \emph{units}, not arithmetic width,
were the binding choice. A Tofino prototype remains future work.
...
observations, law, and epochs, private explicit-rate state still freezes arrival-order
asymmetry; a shared register erases it. \hpccfs carries that
