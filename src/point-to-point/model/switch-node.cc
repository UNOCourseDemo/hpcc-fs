...
TypeId SwitchNode::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SwitchNode")
	...
	.AddAttribute("MaxRtt",
			...
			MakeUintegerChecker<uint32_t>())
	.AddAttribute("FsAlpha",
			"HPCC-FS RCP gain alpha (cc_mode 11)",
			DoubleValue(0.4),
			MakeDoubleAccessor(&SwitchNode::m_fsAlpha),
			MakeDoubleChecker<double>())
	.AddAttribute("FsBeta",
			"HPCC-FS RCP gain beta (cc_mode 11)",
			DoubleValue(0.226),
			MakeDoubleAccessor(&SwitchNode::m_fsBeta),
			MakeDoubleChecker<double>())
	.AddAttribute("FsInitFrac",
			"HPCC-FS initial per-port fair rate as a fraction of link capacity (cc_mode 11)",
			DoubleValue(0.5),
...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p){
	...
	if (1){
		...
		if (buf[PppHeader::GetStaticSize() + 9] == 0x11){ // udp packet
			...
			if (m_ccMode == 3 || m_ccMode == 13){ // HPCC / PowerTCP (same INT record)
				...
				if (m_fairR[ifIndex] <= 0){
					...
					// Avoids the first-interval overload that caused 2-4 PFC events at N>=3
					// (multiple flows all stamping line rate before RCP's first update).
					// RCP converges quickly from here for any other N (solo flow ramps to C in ~1
					// interval; higher incast reduces from C/2).
					m_fairR[ifIndex] = (double)Cbps * m_fsInitFrac;
					m_rcpLastTs[ifIndex] = Simulator::Now().GetTimeStep();
				...
				uint64_t Rstamp = (uint64_t)m_fairR[ifIndex];
				if (ih->GetFairRate() == 0 || Rstamp < ih->GetFairRate())
					ih->SetFairRate(Rstamp);
			}
		}
	}
	m_txBytes[ifIndex] += p->GetSize();
