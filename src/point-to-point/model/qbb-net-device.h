...
public:
	...
	int m_qlast;
	uint32_t m_rrlast;
	Ptr<OldDropTailQueue> m_ackQ; // highest priority queue
...
public:
  ...
  bool Attach (Ptr<QbbChannel> ch);
   ...
   void TriggerTransmit(void);
	...
	TracedCallback<uint32_t, uint32_t> m_tracePfc; // type: 0 = resume, 1 = pause; qIndex
    typedef void (*PfcTracedCallback)( uint32_t, uint32_t);
    typedef void (*PacketQpTracedCallback)( Ptr<const Packet>, Ptr<RdmaQueuePair>);
protected:
