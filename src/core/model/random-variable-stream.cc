...
// 1. Declare a static pointer (initially null)
static Ptr<UniformRandomVariable> g_rng = 0;

// 2. Create a "Getter" function
Ptr<UniformRandomVariable> UniformRandomVariable::GetGlobalRng()
{
    if (g_rng == nullptr)
    {
        // Only creates the object the first time this is called
        g_rng = CreateObject<UniformRandomVariable> ();

        // Optional: Fix the stream index if you need absolute control
        // g_rng->SetStream (50);
    }
    return g_rng;
}
